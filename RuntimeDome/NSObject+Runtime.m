...
+ (NSArray *)propertiesInfo{
    ...
    {
        [properArray addObject:({
            
            NSDictionary *dictionary = [self dictionaryWithProperty:propertys[i]];
...
-(NSArray*)instanceMethodList{
    ...
    Method *metods = class_copyMethodList([self class], &count);
    for (int i = 0; i<count; i++) {
        SEL name = method_getName(metods[i]);
...
+ (void)SwizzlingInstanceMethodWithOldMethod:(SEL)oldMethod newMethod:(SEL)newMethod{
    ...
    BOOL add = class_addMethod(class, originalSelector,
