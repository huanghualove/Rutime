...
- (void)properties {
    NSLog(@"==================属性========================\n\n\n");
    NSLog(@"属性列表%@\n\n\n",[RuntimeObject propertiesInfo]);
    NSLog(@"格式化了的属性列表%@\n\n\n",[RuntimeObject propertiesWithCodeFormat]);
}


- (void)ivar {
    NSLog(@"=================成员变量=======================\n\n\n");
    NSLog(@"成员变量列表%@\n\n\n",[RuntimeObject ivarInfo]);
}


- (void)instanceMethod {
    NSLog(@"==================实例方法======================\n\n\n");
    NSLog(@"实例方法列表%@\n\n\n",[RuntimeObject instanceMethodList]);
}


- (void)classMethod {
    NSLog(@"==================类方法========================\n\n\n");
    NSLog(@"类方法列表%@\n\n\n",[RuntimeObject classMethodList]);
}


- (void)protocol {
    NSLog(@"===================协议=========================\n\n\n");
    NSLog(@"协议列表%@\n\n\n",[RuntimeObject protocolList]);
}
...
- (void)addMethod {
    ...
    RuntimeObject *test1 = [[RuntimeObject alloc] init];
    [test1 performSelector:@selector(hh_addMethod)];
    
    // 2.如果找不到方法，就添加一个addMethod来执行
    id test2 = [[RuntimeObject alloc] init];
    [test2 length];
}
...
- (void)privateMethod {
    NSLog(@"=================调用私有方法=====================\n\n\n");
    RuntimeObject *test = [[RuntimeObject alloc] init];
